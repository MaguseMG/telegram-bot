...
TELEGRAM_BOT_TOKEN = "7615415585:AAGGjqHaSxHENpnP8qQfJ0wW9jz-p4NfHtc"
...
# --- Функция для разбиения длинного текста ---
def split_message(text, max_length=4096):
    ...
    while text:
        if len(text) <= max_length:
            parts.append(text)
            break
        # Попытаемся разбить по переносам строк
...
# --- Функции для постоянного хранения данных (JSON) ---
...
def start(update: Update, context: CallbackContext) -> int:
    ...
    if chat_id in all_data:
        ...
        context.user_data["cabinets"] = []          # Список кабинетов: dict с ключами "name" и "key"
        context.user_data["tracking"] = {}          # Настройки отслеживания: {cabinet_name: bool}
        context.user_data["campaign_states"] = {}   # Состояния кампаний
...
def add_cabinet_key(update: Update, context: CallbackContext) -> int:
    ...
    context.user_data.setdefault("tracking", {})[cabinet_name] = False  # отслеживание выключено по умолчанию
